...
describe("when user is signed in", () => {
  // createSever() --> GET '/apt/user' --> {user: {id: 1, email: 'test@test.com'}}
  createServer([
    {
      path: "/api/user",
      res: () => {
        console.log("LOGGED IN RESPONSE");
        return { user: { id: 1, email: "sadfsda.test.com" } };
      },
    },
  ]);
...
describe("when usr is not signed in", () => {
  // createSever() --> GET '/apt/user'
  createServer([
    {
      path: "/api/user",
      res: () => {
